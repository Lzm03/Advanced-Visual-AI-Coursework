...
class DeepUnfoldingModel(nn.Module):
    def __init__(self, num_iter=5):
        super(DeepUnfoldingModel, self).__init__()
...
def get_subset(dataset, subset_size=20):
...
def train_deep_unfolding_model(model, optimizer, train_loader, num_epochs=1, checkpoint_interval=50):
    model.train()
...
def evaluate_deep_unfolding_model(model, valid_loader):
    model.eval()
    total_psnr, total_ssim, total_lpips = 0, 0, 0

    with torch.no_grad():
        for lr_images, hr_images in valid_loader:
            lr_images, hr_images = lr_images.to(device, dtype=torch.float32), hr_images.to(device, dtype=torch.float32)
            output = model(lr_images)
            avg_psnr, avg_ssim, avg_lpips = calculate_metrics(output.cpu(), hr_images.cpu())
            total_psnr += avg_psnr
            total_ssim += avg_ssim
...
def save_checkpoint(model, optimizer, epoch, checkpoint_dir="./checkpoints"):
    ...
    checkpoint_path = os.path.join(checkpoint_dir, f"unfolding_epoch_{epoch}.pth")
...
if __name__ == '__main__':
    model, optimizer = initialize_deep_unfolding_model()
    model = model.to(device)
    checkpoint_path = "./checkpoints/unfolding_epoch_400.pth"
    start_epoch = 0
    if os.path.exists(checkpoint_path):
        model, optimizer, start_epoch = load_checkpoint(checkpoint_path, model, optimizer)
        model = model.to(device)

    # trained_model = train_deep_unfolding_model(model, optimizer, train_loader, num_epochs=500 - start_epoch)

    # evaluate_deep_unfolding_model(model, valid_loader)
